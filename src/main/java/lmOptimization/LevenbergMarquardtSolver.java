...
import Jama.Matrix;
import filamentEnders.MTFitFunction;
import ij.IJ;
...
public class LevenbergMarquardtSolver {
	 ...
	 */
	public static final double chiSquared(final double[][] x, final double[] a, final double[] b, final double[] y,
	...
	/**
	 * Minimize E = sum {(y[k] - f(x[k],a)) }^2 Note that function implements the
	 * value and gradient of f(x,a), NOT the value and gradient of E with respect to
	 * a!
	 * 
	 * @param x
	 ...
	 * @param lambda
	 *            blend between steepest descent (lambda high) and jump to bottom of
	 *            quadratic (lambda zero). Start with 0.001.
	 * @param termepsilon
	 ...
	 */
	public static final int solve(double[][] x, double[] a, double[] b, double[] y, MTFitFunction f, double lambda,
			double termepsilon, int maxiter) throws Exception {
		int npts = y.length;
		int nparm = a.length;
		...
		do {
			...
			// New truncation criteria to fasten the solver when it moves slowly during an
			// iteration step @Varun
			for (int i = 0; i < nparm; i++) {
